    ...
    <li><span class='badge grade-5'>5</span> The Active Directory infrastructure correctly implements the latest state-of-the-art administrative model and security features.</li>
		...
		protected void GenerateUserInformation()
		{
			...
			if (Report.ListHoneyPot != null && Report.ListHoneyPot.Count > 0)
			{
				...
				AddParagraph("A honey pot has been configured. It is used to generate fake security issues that are heavily monitored and that a hacker will spot using security tools like RisX. By enabling this feature, all the accounts listed below will not be evaluated with PingCastle rules.");
